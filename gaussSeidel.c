...
void gaussSeidel(int dim, float** A, float* B, float* X, float precisao);
...
int main(int argc, char *argv[]) {
    ...
    for (int sistema = 0; sistema < N_SISTEMAS; sistema++) {
        ...
        clock_t start = clock();
        gaussSeidel(DIMENSAO, A, B, X, PRECISAO);
        clock_t end = clock();

        // Imprimindo as soluções
        imprimirSolucoes(DIMENSAO, X, "GAUSS-SEIDEL");
        printf("Tempo de execucao do Metodo de Gauss-Seidel: %f segundos\n", ((double)(end - start)) / CLOCKS_PER_SEC);
